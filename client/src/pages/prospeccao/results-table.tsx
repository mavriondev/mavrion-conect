...
import {
  Building2, MapPin, Briefcase, AlertCircle, Download, Loader2, Filter, XCircle
} from "lucide-react";
...
export default function ResultsTable({
  ...
  return (
    <div className="lg:col-span-2 space-y-3">
      ...
      {searchMeta && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            ...
            {searchResults && searchResults.filter(r => r.alreadySaved).length > 0 && (
              <span className="ml-1 text-muted-foreground">
                ({searchResults.filter(r => r.alreadySaved).length} já importadas — ocultas)
              </span>
            )}
          </span>
        </div>
      ...
      {!searchLoading && searchResults && searchResults.filter(r => !r.alreadySaved).length === 0 && (
        <Card>
      ...
      {!searchLoading && searchResults &&
        searchResults
          .filter(r => !r.alreadySaved)
          .map(result => (
            <Card key={result.taxId} data-testid={`card-result-${result.taxId}`}>
