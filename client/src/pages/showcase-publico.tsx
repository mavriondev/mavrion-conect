...
function MetricCard({ label, value, icon: Icon, color }: { label: string; value: any; icon?: any; color?: string }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
      ...
      <p className="text-lg font-bold text-gray-900 break-all overflow-hidden" title={typeof value === 'string' ? value : undefined}>{typeof value === 'string' && value.length > 20 ? <span className="text-xs leading-snug">{value}</span> : value}</p>
    </div>
  );
}
